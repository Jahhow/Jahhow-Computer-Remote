...
public class MotionMouseFragment extends Fragment implements SensorEventListener {
	...
	public void onSensorChanged(SensorEvent event) {
		if (hasLastRotateZ) {
			...
			MetrixMult(rotationMatrix, mainVector, rotatedVector);
			...
			float diffAcosZdp = (float) ((Math.acos(z) - originAcosZ) * speed /** Math.PI / 2*/);// *PI/2 is for converting unit to be as adjustedDiffRotateZ
			float adjFactor = (float) GetAdjustFactor(diffRotateZdp, diffAcosZdp, moveMouseAdjExp);
			int roundAdjDiffRotateZdp = Math.round(adjFactor * diffRotateZdp);
			int roundAdjDiffAcosZdp = Math.round(adjFactor * diffAcosZdp);
			if (roundAdjDiffRotateZdp != 0 || roundAdjDiffAcosZdp != 0) {
				mainActivity.SendMouseMove((short) roundAdjDiffRotateZdp, (short) roundAdjDiffAcosZdp);
				originRotateZ += roundAdjDiffRotateZdp / (adjFactor * speed);
				originAcosZ += roundAdjDiffAcosZdp / (adjFactor * speed);
			}
		} else {
			SensorManager.getRotationMatrixFromVector(rotationMatrix, event.values);
			MetrixMult(rotationMatrix, mainVector, rotatedVector);
	...
	void MetrixMult(float[] leftM, float[] vector, float[] result) {
